...
<div class="bg-white rounded-lg shadow-md p-6">
    <h2 class="text-xl font-bold text-gray-800 mb-4">Edit Mata Kuliah</h2>

    <form method="POST" action="{{ route('admin.mata-kuliah.update', $mataKuliah->id) }}" class="space-y-4">
        @csrf
        @method('PUT')
        <div>
            <label class="block text-sm font-medium text-gray-700">Kode MK</label>
            <input type="text" name="kode_mk" value="{{ old('kode_mk', $mataKuliah->kode_mk) }}" class="mt-1 w-full border rounded px-3 py-2" required>
        ...
        <div>
            ...
            <input type="text" name="nama_mk" value="{{ old('nama_mk', $mataKuliah->nama_mk) }}" class="mt-1 w-full border rounded px-3 py-2" required>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                ...
                <input type="number" name="sks" min="1" max="10" value="{{ old('sks', $mataKuliah->sks) }}" class="mt-1 w-full border rounded px-3 py-2" required>
                @error('sks')<p class="text-red-600 text-sm mt-1">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700">Semester</label>
                <input type="number" name="semester" min="1" max="14" value="{{ old('semester', $mataKuliah->semester) }}" class="mt-1 w-full border rounded px-3 py-2" required>
            ...
            <div>
                ...
                <select name="program_studi_id" class="mt-1 w-full border rounded px-3 py-2" required>
                    @foreach($prodiList as $prodi)
                        <option value="{{ $prodi->id }}" {{ old('program_studi_id', $mataKuliah->program_studi_id) == $prodi->id ? 'selected' : '' }}>{{ $prodi->nama_prodi }}</option>
                    @endforeach
                </select>

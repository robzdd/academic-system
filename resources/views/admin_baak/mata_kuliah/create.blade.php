...
<div class="bg-white rounded-2xl shadow-lg border border-gray-200 p-6">
    ...
    <form method="POST" action="{{ route('admin.mata-kuliah.store') }}" class="space-y-5">
        @csrf
        <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                ...
                <input type="number" name="sks" min="1" max="10" value="{{ old('sks', 3) }}"
                    class="mt-1 w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-indigo-600 focus:outline-none" required>
                @error('sks')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700">Semester</label>
                <input type="number" name="semester" min="1" max="14" value="{{ old('semester', 1) }}"

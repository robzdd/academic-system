...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 bg-white/90 backdrop-blur-sm rounded-2xl shadow-md p-6 border border-gray-100">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
            <div>
                <h2 class="text-2xl font-semibold text-gray-800">Jadwal Kuliah</h2>
                <p class="text-gray-500 text-sm">
                    Anda memiliki <span class="font-semibold text-blue-600">{{ $jadwalHariIni->count() }}</span> aktivitas hari ini
                </p>
            </div>
            <form method="GET" action="">
                <input type="date" id="tanggal" name="tanggal" value="{{ $tanggalDipilih }}"
                    class="border border-gray-300 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-150"
        ...
        @if($jadwalHariIni->count() > 0)
            @foreach($jadwalHariIni as $jadwal)
                <div class="border border-blue-400/60 bg-blue-50/40 rounded-xl p-5 mb-4 shadow-sm hover:shadow-md transition-all duration-200">
                    <div class="flex flex-col md:flex-row md:justify-between md:items-start">
                        ...
                        <div class="mt-4 md:mt-0">
                            <button class="flex items-center justify-center gap-2 border border-blue-400 bg-blue-100 hover:bg-blue-200 text-blue-800 px-4 py-2 rounded-lg text-sm font-medium transition-colors duration-200">
                                Opsi Lainnya
        ...
        @else
            <div class="flex flex-col items-center justify-center py-16 text-gray-400">
                <img src="https://cdn-icons-png.flaticon.com/512/4076/4076432.png" alt="no schedule"
            ...
            <div class="grid grid-cols-7 text-center mt-2">
                ...
                @for ($tanggal = 1; $tanggal <= $jumlahHari; $tanggal++)
                    ...
                    <form method="GET" action="{{ route('mahasiswa.dashboard') }}">
                        ...
                        <button type="submit"
                            class="w-10 h-10 m-1 rounded-full font-medium text-sm
                            {{ $isSelected ? 'bg-blue-500 text-white' :
                               ($isToday ? 'border border-blue-400 text-blue-600' : 'hover:bg-gray-100 text-gray-700') }}">
                            {{ $tanggal }}
                        </button>
                    </form>
                @endfor
